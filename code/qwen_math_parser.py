...
def _fix_fracs(string):
    """Fix the LaTeX fraction notation in the string."""
...
def _fix_a_slash_b(string):
    r"""Fix the a/b notation in the string to LaTeX \frac{a}{b}."""
...
def _fix_sqrt(string):
    """Fix the square root notation in the string."""
...
def strip_string(string, skip_unit=False):
    """Strip the string from LaTeX formatting and clean it up."""
...
def extract_multi_choice_answer(pred_str):
    """Extract the answer from the model output for multiple choice questions."""
    # TODO: SFT models
...
def choice_answer_clean(pred: str):
    """Clean the answer from the model output."""
    pred = pred.strip("\n")
...
def find_box(pred_str: str):
    """Find the boxed answer in the model output."""
...
def choice_answer_clean2(pred: str) -> str:
    """Clean the answer from the model output."""
    pred = pred.strip("\n").rstrip(".").rstrip("/").strip(" ").lstrip(":")
    # Clean the answer based on the dataset
    tmp = re.findall(r"\b(A|B|C|D|E)\b", pred.upper())[0]
    if tmp:
        pred = tmp
    else:
        pred = [pred.strip().strip(".")]
...
def parse_digits(num):
    """Parse a string to a float or percentage."""
...
def math_equal(
    ...
    if (
        ...
        and choice_answer_clean2(prediction) == reference
    ):
        return True
...
def numeric_equal(prediction: float, reference: float):
    """Check if two numbers are numerically equal."""
...
def symbolic_equal_process(a, b, output_queue):
    """Process function to check symbolic equality."""
